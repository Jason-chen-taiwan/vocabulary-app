...
describe('BuiltinWordBookSource', () => {
  ...
  it('listCards resolves the book by slug then returns its words with examples', async () => {
    const repo = {
      ...
      listWordsByBookWithExamples: vi.fn().mockResolvedValue([{ id: 'w1', headword: 'invoice', phonetic: null, partOfSpeech: 'n.', definitionZh: '發票', examTags: [], examples: [{ id: 'e1', sentence: 'X.', translationZh: 'X。', source: null }] }]),
    }
    ...
    expect(repo.listWordsByBookWithExamples).toHaveBeenCalledWith('b1')
    expect(cards).toHaveLength(1)
    expect(cards[0].examples[0].sentence).toBe('X.')
  })

  it('listCards returns [] when the book does not exist', async () => {
    const repo = { getWordBookBySlug: vi.fn().mockResolvedValue(null), listWordsByBookWithExamples: vi.fn() }
    const src = new BuiltinWordBookSource('missing', repo as any)
    expect(await src.listCards()).toEqual([])
    expect(repo.listWordsByBookWithExamples).not.toHaveBeenCalled()
  })
})
